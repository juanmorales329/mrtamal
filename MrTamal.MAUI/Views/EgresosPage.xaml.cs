...
{
    private readonly EgresosViewModel _vm;

    public EgresosPage(EgresosViewModel vm)
    {
        InitializeComponent();
        _vm = vm;
    }
    ...
    private async Task MostrarFormulario(MovimientoDto? item)
    {
        ...
        {
            await DisplayAlert("Sin catálogos", "Primero crea tipos de egreso en Catálogos.", "OK");
            return;
        }

        var opciones = _vm.Catalogos.Where(c => c.Activo).Select(c => $"[{c.Codigo}] {c.Descripcion}").ToArray();
        var seleccion = await DisplayActionSheet("Tipo de egreso", "Cancelar", null, opciones);
